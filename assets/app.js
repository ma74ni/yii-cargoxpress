...
$(document).ready(function () {
  ...
  $.validate({
    ...
    onSuccess: function () {
      ...
      const params = $('#frm_orden').serialize();
      const action = $('#frm_orden #action').val();
      const button = $('#frm_orden button[type="submit"]');
    ...
    sendOrder: function (params) {
      ...
      formatedParams.orden_id
        ? (temp += 'Código: ' + formatedParams.orden_id + '\n')
        : '';
      temp += `Origen: ${
      ...
      if (data.code == 1) {
        switch (action) {
          ...
          case 'getDetalleOrden':
            ...
            $('.cambiar-estado-orden').attr(
              'data-modal_detalle',
              'detalle-orden-modal'
            );
            $('.cambiar-estado-orden').attr(
              'data-modal_codigo_orden',
              data.details.codigo_orden
            );
            $('.cambiar-estado-orden').attr(
              'data-modal_new',
              'change-status-orden-modal'
            );
            ...
            $('.cambiar-fecha-entrega').attr(
              'data-modal_new',
              'cambiar-fecha-entrega-orden-modal'
            );
            ...
            $('.cambiar-fecha-envio').attr(
              'data-modal_new',
              'cambiar-fecha-envio-orden-modal'
            );
            ...
            if (data.msg == 'OK') {
              Swal.fire({
                ...
                if (result.value) {
                  ...
                  callAjax(
                    'changeAsignarOrden',
                    'orden_id=' + data.details.orden_id + '&id_ruta=' + id_ruta
                  );
                }
              });
            } else {
          ...
          case 'getDetalleRuta':
            ...
            $('.administrar-ordenes').attr(
              'data-fecha_ruta',
              data.details.fecha_ruta
            );
            $('.administrar-ordenes').attr(
              'data-detalle',
              data.details.detalle
            );
            $('.administrar-ordenes').attr(
              'data-id_mensajero',
              data.details.id_mensajero
            );
            $('.administrar-ordenes').attr(
              'data-mensajero',
              data.details.nombres
            );
            $('.administrar-ordenes').attr(
              'data-foto_perfil_url',
              data.details.foto_perfil_url
            );
