$(document).ready(function () {
  ...
  $.validate({
    ...
    onSuccess: function () {
      ...
      if ($('#seleccionado_' + codigo_orden_busqueda).exists()) {
        $(
          'input:checkbox[id="seleccionado_' + codigo_orden_busqueda + '"]'
        ).prop('checked', true);
      } else {
    ...
    onSuccess: function () {
      if (buttonpressed == 'guardar') {
        Swal.fire({
          ...
          if (result.value) {
            var params = $(
              '#frm_table_todas_ordenes_masivo_estado'
            ).serialize();
            var action = $(
              '#frm_table_todas_ordenes_masivo_estado #action2'
            ).val();
            var button = $(
              '#frm_table_todas_ordenes_masivo_estado button[type="submit"'
            );
  ...
  function initTableMasivoEstadoBusquedaAvanzada() {
    var params = $(
      '#frm_table_todas_ordenes_masivo_estado_busqueda'
    ).serialize();
    var action = $(
      '#frm_table_todas_ordenes_masivo_estado_busqueda #action'
    ).val();
    params += '&language=' + language;
    if ($.fn.dataTable.isDataTable('#table_list')) {
      table = $('#table_list').DataTable();
